...
def main(argv):
    ...
    for lang in ["pol"]:
        ...
        if TEST:
            devlines = [line.strip() for line in open(path + lang_test + ".tst", "r", encoding='utf8') if line != '\n']
        numcorrect = 0
        numguesses = 0
        if OUTPUT:
            outfile = open(path + lang_test + ".out", "w", encoding='utf8')
        for l in devlines:
...
#                    lemma, msd, = l.split(u'\t')
        ...
        print(lang_test + ": " + str(str(numcorrect/float(numguesses)))[0:7])
